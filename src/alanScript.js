// Main
intent('Give me the news from $(item* (.*)) category', (p) => {
    if(p.item.value==='general' || p.item.value==='health'||p.item.value==='business'||p.item.value==='politics'||p.item.value==='sports'||p.item.value==='entertainment'||p.item.value==='technology'){
        p.play({command:"HappyMorning News",data:p.item.value})
        p.play(`Fetching News on ${p.item.value} category.`)
    }else{
        p.play(`Data not available,please try for other categories.`)
    }
})
